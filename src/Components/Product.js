...
/////////////////////////////////////////////////////////////////////////////
    ...
    <div className="w-full max-w-sm mx-auto rounded-md shadow-md overflow-hidden">
        <div className="flex items-end justify-end h-56 w-full bg-cover bg-center" style={{backgroundImage:`url(${props.image})`}}>
                        <button onClick={ (e) => {
                            const classes =["bg-blue-600","hover:bg-blue-500","focus:bg-blue-500","bg-green-600","hover:bg-green-500","focus:bg-green-500"]
                            addToCart()
                            e.target.childNodes[0].classList.toggle("hidden")
                            e.target.childNodes[1].classList.toggle("hidden")
                            classes.map(el => e.target.classList.toggle(el))
                            setTimeout( () => {
                            e.target.childNodes[0].classList.toggle("hidden")
                            e.target.childNodes[1].classList.toggle("hidden")
                            classes.map(el => e.target.classList.toggle(el))
                            },500)
                        }} className="p-2 rounded-full text-white mx-5 -mb-4 bg-blue-600  hover:bg-blue-500 focus:outline-none focus:bg-blue-500">
